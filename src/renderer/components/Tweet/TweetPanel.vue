<template>
  <div class="tweet-panal">
    <div class="panel-left">
      <TweetList ref="homePanel" :panelName="'home'" v-show="selectPanelName == 'home'" v-bind:options="this.$store.state.DalsaeOptions.uiOptions" v-bind:tweets="this.$store.state.tweets.home" />
      <TweetList
        ref="mentionPanel"
        :panelName="'mention'"
      ...
      />
      <TweetList ref="favPanel" :panelName="'fav'" v-show="selectPanelName == 'fav'" v-bind:options="this.$store.state.DalsaeOptions.uiOptions" v-bind:tweets="this.$store.state.tweets.fav" />
      <TweetList
        ref="daehwaPanel"
        :panelName="'daehwa'"
  ...
  mounted: function() {
    ...
    this.EventBus.$on("TweetKeyDown", e => {
      if (e.keyCode == 49) {
        ...
        if (this.$store.state.tweets.daehwa[0].orgTweet.in_reply_to_status_id_str) {
          //캐시 정리가 끝난 후 대화가 있을 경우에 api 콜
        ...
        //backspace, 대화 숨기기. vuex의 대화 목록을 clear
        e.preventDefault();
        ...
        //h, 해시태그, 해시 미구현
      } else if (e.keyCode == 127 || e.keyCode == 46) {
        //del, 트윗 삭제
        e.preventDefault();
        var tweet = this.selectPanel.GetSelectTweet();
    ...
    this.EventBus.$on("ArrowUp", e => {
      //e: event
      this.selectPanel.Prev(e);
    });
    this.EventBus.$on("ArrowDown", e => {
      //e: event
      this.selectPanel.Next(e);
    });
    this.EventBus.$on("TweetFocus", id => {
      //id: tweet id
    ...
    this.EventBus.$on("FocusPanel", selectPanelName => {
      if (this.selectPanelName == "daehwa" && selectPanelName != "daehwa") {
        //대화패널에서 나갈 경우 클리어
        this.$store.dispatch("ClearDaehwa");
      }
